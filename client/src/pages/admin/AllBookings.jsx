...
//       doc.setFontSize(10);
//       doc.text("Thank you for choosing our premium travel services!", pageWidth / 2, 225, { align: "center" });
...
const AllBookings = () => {
  ...
  const handleDownloadReceipt = (booking) => {
    ...
    img.onload = () => {
      ...
      doc.setFontSize(12);
      doc.setFont("normal");
      doc.text(`Customer Name: ${booking?.buyer?.username}`, 10, 80);
      doc.text(`Email: ${booking?.buyer?.email}`, 10, 90);
      doc.text(`Phone: ${booking?.buyer?.phone || 'N/A'}`, 10, 100);
      doc.text(`Destination: ${booking?.packageDetails?.packageDestination}`, 10, 110);
      doc.text(`Date: ${booking?.date}`, 10, 120);
      doc.text(`Persons: ${booking?.persons}`, 10, 130);
