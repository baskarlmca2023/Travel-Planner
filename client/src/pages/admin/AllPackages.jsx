// //src/pages/admin/AllPackages.jsx
...
const AllPackages = () => {
  ...
  // Fetch packages based on the current filter and search term
  const getPackages = async () => {
    setPackages([]);
    try {
      setLoading(true);
      let url =
        filter === "offer" // offer filter
          ? `/api/package/get-packages?searchTerm=${search}&offer=true`
          : filter === "latest" // latest filter
          ? `/api/package/get-packages?searchTerm=${search}&sort=createdAt`
          : filter === "top" // top-rated filter
          ? `/api/package/get-packages?searchTerm=${search}&sort=packageRating`
          : `/api/package/get-packages?searchTerm=${search}`; // all packages
          
      console.log("Fetching packages from:", url); // Debugging log
      const res = await fetch(url);
      const data = await res.json();

      if (data?.success) {
        console.log("Packages fetched:", data.packages); // Debugging log
        setPackages(data?.packages);
        setLoading(false);
      } else {
        setLoading(false);
        alert(data?.message || "Something went wrong while fetching packages!");
      }

      // If more than 8 packages, show "Show More" button
      if (data?.packages?.length > 8) {
        setShowMoreBtn(true);
      } else {
        setShowMoreBtn(false);
      ...
      setLoading(false);
      console.error("Error fetching packages:", error); // Error logging
    }
  };
  ...
  const onShowMoreSClick = async () => {
    ...
    let url =
      filter === "offer" // offer filter
        ...
        : filter === "top" // top-rated filter
        ? `/api/package/get-packages?searchTerm=${search}&sort=packageRating&startIndex=${startIndex}`
        : `/api/package/get-packages?searchTerm=${search}&startIndex=${startIndex}`; // all packages
        
    console.log("Fetching more packages from:", url); // Debugging log
    const res = await fetch(url);
    const data = await res.json();
    if (data?.packages?.length < 9) {
      setShowMoreBtn(false);
  ...
  const handleDelete = async (packageId) => {
    try {
      ...
      const data = await res.json();
      alert(data?.message);
      getPackages(); // Reload packages after deletion
      setLoading(false);
    } catch (error) {
      setLoading(false);
      console.error("Error deleting package:", error); // Error logging
    }
  };
